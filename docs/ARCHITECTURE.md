...
This document describes how Tolka is put together internally and where the boundary runs
between Tolka and its primary consumer, [eneo](https://github.com/eneo-ai/eneo). The
README covers the *what* (API, engine tiers, configuration); this covers the *how* and
the *why of the split*.

## System overview

Tolka is a single-purpose service: audio in, speaker-labelled transcript out, asynchronously.
...
flowchart LR
    ...
    subgraph tolka["Tolka deployment"]
...
storage first. `TOLKA_RUN_WORKER=true` collapses both into one process for development.
...
├── config.py           all TOLKA_* settings (pydantic-settings)
...
└── pipeline/
    ├── factory.py      TOLKA_ENGINE → engine implementation
...
under Tolka's control, while ASR text production is pluggable (in-process, remote endpoint,
or skipped entirely for `task=diarize`).

## Job lifecycle

1. **Admission** (`api/jobs.py`): authenticate → validate the request against the deployment
   tier (e.g. `task=transcribe` on `TOLKA_ENGINE=diarize` → 422) → enforce
   `TOLKA_MAX_QUEUED_JOBS` / `..._PER_CLIENT` (429, pre-body) → persist the job `queued`
...
4. **Terminal**: result rows, webhook outbox events, and status flip in one transaction.
   ...
   after `TOLKA_RETENTION_HOURS` by the worker's purge loop.
...
or a segment-level merge (`remote`, and `TOLKA_DIARIZE_PREFER_ALIGN=false`, are the
...
- **Exclusive diarization** (`TOLKA_DIARIZE_EXCLUSIVE`, default on): words are attributed
...
Every threshold above is env-tunable via the `TOLKA_ATTR_*` settings (see
`AttributionTuning` in `pipeline/diarize.py`); the defaults are the tested behaviour.

## Eneo and Tolka: the responsibility split

Eneo is a multi-tenant AI platform; Tolka is its transcription/diarization engine. The
boundary is drawn so that each side owns what only it can know:

| Concern | Owner | Notes |
| --- | --- | --- |
| End-user identity, tenants, permissions | **eneo** | Tolka sees exactly one credential (`client_id=eneo`) for all of eneo's tenants |
| UI, flows, orchestration, retries, deadlines | **eneo** | Tolka re-leases crashed jobs forever; eneo's poll deadline is the backstop |
| Original audio custody & long-term storage | **eneo** | Tolka keeps audio only while a job is active, results only for `TOLKA_RETENTION_HOURS` |
| ASR in eneo's own transcription flow | **eneo** | eneo may transcribe with its own models and use Tolka for labels only (`task=diarize`) |
...
| Job queueing, admission, leasing, crash recovery | **Tolka** | PostgreSQL leases; per-client caps bound eneo's total fan-in across tenants |
| Whisper ASR when asked to transcribe | **Tolka** | in-process on GPU (`local`) or brokered to an OpenAI-compatible endpoint (`hybrid`/`remote`) |
| Word timestamps | **Tolka** | audio-derived CTC forced alignment wins over anything a provider or caller decoded |
| Speaker diarization & attribution | **Tolka** | pyannote always local; attribution/smoothing/grouping identical for both tasks |
| Rendered output contract | **Tolka** | `TranscriptionResult` + the `[HH:MM:SS - HH:MM:SS] SPEAKER_00:` text format |

Two integration paths use the same deployment, chosen per request:

1. **Tolka transcribes** — eneo uploads audio (`task=transcribe`), Tolka runs the full
   pipeline and returns text with speakers.
2. **Eneo transcribes, Tolka labels** — eneo runs its own ASR in chunks, then submits the
   audio plus the chunk texts as measured segment windows (`task=diarize`). Tolka
...
In both paths eneo passes `result.text` verbatim into its flow output — Tolka owns the
...
### What Tolka deliberately does not do

- **Multi-tenancy** — one consumer = one credential; fairness between eneo's tenants is
  eneo's problem, capacity between consumers is Tolka's (`TOLKA_MAX_QUEUED_JOBS_PER_CLIENT`).
- **Interactive auth** — static server-to-server bearer tokens only; identity-aware
  ingress and OAuth live in front of Tolka, not inside it.
...
- **Trusting foreign timestamps** — the plausibility gate and `forced`-first policy exist
  ...
  `TOLKA_DIARIZE_PREFER_ALIGN=false`), never by silent degradation.
