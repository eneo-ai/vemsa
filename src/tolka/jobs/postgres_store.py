...
_CLAIM_JOB_COLUMNS = (
    "jobs.id, jobs.client_id, jobs.status, jobs.created_at, jobs.updated_at, "
...
class PostgresJobStore:
        ...
        *,
        ...
        async with self.pool.acquire() as connection, connection.transaction():
            command = await connection.execute(
                ...
                " lease_owner = NULL, lease_expires_at = NULL WHERE id = $4" + owner_clause,
                *values,
            )
    ...
    async def fail(
        ...
        *,
        ...
        owner_clause = "" if worker_id is None else " AND lease_owner = $5"
        values: list[Any] = [JobStatus.FAILED.value, error, datetime.now(UTC), job_id]
        if worker_id is not None:
            values.append(worker_id)
        async with self.pool.acquire() as connection, connection.transaction():
            command = await connection.execute(
    ...
    async def purge_older_than(self, cutoff: datetime) -> list[Job]:
        rows = await self.pool.fetch(
            f"DELETE FROM jobs WHERE status IN ($1, $2) AND updated_at < $3"
