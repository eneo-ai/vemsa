...
Companion plan on the Eneo side: `eneo/.claude/plans/tolka-diarize-only.md`.

## Context

Eneo wants its own model registry (per-tenant providers, credentials, governance) to do
the ASR, and Tolka to do only what its registry cannot: speaker diarization. Today the
...
(`src/tolka/pipeline/diarize.py:174`) needs only audio, and
`resolve_segments(words, segments, turns)` (`diarize.py:102`) plus `render_text`
(`src/tolka/pipeline/render.py:9`) are pure functions that do not care where the words
came from. `HybridEngine` (`src/tolka/pipeline/hybrid.py:39-58`) is a hand-composed
...
no engine (and therefore no `TOLKA_WHISPER_API_BASE`) required for diarize-only
deployments.

## 1. Request model (`src/tolka/jobs/models.py`)
...
- `model_validator(mode="after")`: `task == "diarize"` requires `words` or `segments`
  ...
  `"external"` when absent on a diarize job so the result never claims Tolka's default
  model ran.
...
## 2. Submission (`src/tolka/api/jobs.py`)
...
## 3. Execution (`src/tolka/jobs/queue.py`)
...
- Give the queue a `Diarizer` dependency directly (constructor-injected, duck-typed like
  ...
  `src/tolka/pipeline/factory.py` build the `Diarizer` once and share it with the engine
...
- `Diarizer.load()` must be part of readiness for every engine tier now, including a
  new `TOLKA_ENGINE=diarize` tier (see 5) that constructs no ASR engine at all.
...
- `jobs.request_json` now carries transcripts. No migration (JSON column), but:
  - The retention purge (`TOLKA_RETENTION_HOURS`, `queue.py:281-309`) already deletes
...
- New `tests/test_diarize_task.py`: multipart submit with a `words` JSON part; JSON
  ...
  `diarize` engine tier readiness without `TOLKA_WHISPER_API_BASE`.
...
2. Local: `TOLKA_ENGINE=fake`, submit `curl -F file=@a.mp3 -F task=diarize
   -F 'words=[{"word":"hej","start":0.0,"end":0.4}]'`, poll, fetch result; `text` has
   speaker prefixes and `model == "external"`.
3. GPU box: `TOLKA_ENGINE=diarize` with a real HF token, a 10 min two-speaker file and a
   transcript produced by Eneo's adapter; compare speaker turns against the `hybrid`
   tier's output on the same file.
4. `readyz` on the `diarize` tier without `TOLKA_WHISPER_API_BASE` set.
