...
Eneo (adjacent repo) is integrating Vemsa as the transcription engine for its flow
...
`eneo/.claude/plans/vemsa-flow-transcription.md`.

## What eneo's v1 needs from Vemsa (config/deployment only — no code changes identified)

1. **Token provisioning**: a named credential for eneo, e.g. `VEMSA_API_TOKENS=eneo=<secret>`; eneo stores it as `FLOW_TRANSCRIPTION_SERVICE_API_KEY`.
2. **Deployment reachability**: the eneo flow execution worker must reach Vemsa's API. Vemsa's compose binds `api` to `127.0.0.1` — expose it on the shared network for the eneo deployment (and the devcontainer for local dev). GPU worker sizing per Vemsa's compose (`TORCH_VARIANT`, `ML_EXTRAS`).
3. **Admission-control sizing**: `VEMSA_MAX_QUEUED_JOBS_PER_CLIENT` (default 10) bounds eneo's concurrent submissions; all eneo tenants share one `client_id`, and eneo's flow concurrency is 4 runs/tenant — size for multi-tenant fan-in. 429 is returned before the body is read; eneo treats it as retryable.
4. **Upload limit**: `VEMSA_MAX_AUDIO_BYTES` (default 2 GiB) must be ≥ eneo's max audio upload size (binds on the original compressed file — eneo sends originals, not wav).
5. **Retention**: `VEMSA_RETENTION_HOURS` (default 72) is ample; eneo fetches results within the run (≤1h).
6. **Engine tier**: per deployment (`local` on a GPU box, or `hybrid` with `VEMSA_WHISPER_API_BASE` pointing at existing hosted whisper). Vemsa's own maturity markers: `local`/`hybrid` carry GPU-VERIFY(milestone-2) flags; `remote` is fully CI-exercised.
7. **Expected max job latency** for a 2 GiB file should be characterized so eneo's 3300s poll deadline can be validated (Vemsa worker-crash retries extend wall time unboundedly — see gap 6 below).

## Known gaps in Vemsa that eneo v1 works around (candidate future work, priority order)
...
2. **No cancel endpoint** — an aborted/timed-out eneo step leaves the Vemsa job running to completion (wasted GPU). Future: `DELETE /v1/jobs/{id}`.
3. **No speaker-count hints** — `min_speakers`/`max_speakers` passthrough to pyannote is already marked future in `diarize.py`.
4. **Webhook path unused by v1** — Vemsa's transactional outbox + HMAC webhooks pair with a future eneo async-checkpoint wait model. No Vemsa change needed; noting the pairing.
...
- Diarize-only jobs (added after v1): `task=diarize` on `POST /v1/jobs` with a caller-supplied
  ...
  is validated. Transcript size is capped by `VEMSA_MAX_TRANSCRIPT_BYTES` (default 8 MiB,
  413 over it). The result is the same `TranscriptionResult` shape; `model` is echoed from
  the request and defaults to `"external"`. `VEMSA_ENGINE=diarize` deployments reject
  `task=transcribe` with 422. `diarize=false` with `task=diarize` is a 422.
  Version-skew hazard: a pre-task Vemsa deployment silently ignores the unknown fields and
